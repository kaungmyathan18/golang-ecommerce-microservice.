...
import (
	...
	"github.com/kaungmyathan18/golang-ecommerce-microservice/inventory-service/internal/rabbitmq"
...
func main() {
	...
	cfg := config.DefaultConfig("inventory-service-worker")
	...
	q, err := rmq.DeclareQueue("inventory.order-cancelled", "order.cancelled")
	if err != nil {
		logger.Fatal("queue declare", zap.Error(err))
	}
	msgs, err := rmq.Channel().Consume(q.Name, "inventory-worker", false, false, false, false, nil)
	if err != nil {
...
func handleDelivery(log *zap.Logger, repo *repository.Repository, d amqp.Delivery) {
	...
	if err != nil {
		log.Error("restore stock", zap.Error(err))
		_ = d.Nack(false, true)
		return
	}
	log.Info("stock restored", zap.String("order_id", evt.OrderID), zap.Int("stock", stock))
	_ = d.Ack(false)
}
