...
func DefaultConfig(appName string) *Config {
	...
	cfg.RabbitMQ = RabbitMQConfig{URL: getEnv("RABBITMQ_URL", "amqp://guest:guest@localhost:5672/")}
	cfg.Payment = PaymentConfig{StubDelay: time.Duration(envInt("PAYMENT_STUB_DELAY_SEC", 5)) * time.Second}
	return cfg
}
